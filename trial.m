...
%     final_dat_cell(1,i) = {temp_trend_ar};
    ...
    %plot(temp_trend_ar)
    %figure;
    %plot(temp_calib_pos_ar,temp_en_ar)
    temp_trend_ar = jacobian(temp_en_ar,temp_calib_pos_ar,temp_trend_ar,direc,run_name);
    %plot(temp_trend_ar)
...
function img_psl = psl_calc(img,res,sensit,lat,ran)
    ...
    col=size(data,2);
    %PSLtotal=0;
    % so this is the value that I had read from the paper
    %psl = (h*(10^(L/2))*((data./G).^2)*(Res/100)^2);
    %here is the real formula that had been used in the paper
...
function dNdE = jacobian(en_vals,x_axis,dat,direc,run_name)
    %figure;
    %plot(x_axis)
    %figure
    %plot(en_vals,dat);
    ...
    dEdx = dE./dx;
    figure;
    plot(en_vals(1:end-1),dEdx);
    bleh
    %plot(x_axis(1:end-1),dEdx);
    %dEdx = dE./dx;
    %plot(en_vals(1:end-1),dxdE);
    %size(dxdE)
    %size(dat)
    dNdE = dat(1:end-1)./dEdx;
    plot(en_vals(1:end-1),dNdE)
    save_name = 'jacob_temp';
    col_names = ['x_axis','en_vals','dx','dE','dEdx','dNdE'];
    col_array = [x_axis(1:end-1);en_vals(1:end-1);dx;dE;dEdx;dNdE];
    temp = csv_wr(col_names, col_array, direc,run_name, save_name);
    a = 0;
end
...
function nil = csv_wr(col_names, col_array, direc,run_name, save_name)
    %[m,n] = size(col_array);
    col_array = col_array.';
    %savetable = array2table(col_array,'VariableNames',col_names);
    savetable = array2table(col_array);
    save_name = direc + run_name + save_name + ".csv"; 
    writetable(savetable,save_name);
    nil = 0;
end
